...
const BlogCards = () => {
  ...
  const posts =
    location.pathname === "/search"
      ? state.searchBlogPosts
      : state.blogData.posts;
  return (
    <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 font-futura">
      {posts.map((post) => (
        <li
          key={post.id}
          className="flex flex-col border border-[#eeeeee] m-5 "
        >
          <Link to={`/blog/${post.id}`} state={post} className="flex flex-col">
            <img
            ...
            <div className="flex flex-col px-[15px] my-5">
              ...
              <p className="text-[12px] font-semibold uppercase leading-[20px] text-[#475671]">
